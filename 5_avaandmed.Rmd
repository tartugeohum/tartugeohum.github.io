...
Eesti avaliku sektori avaandmed on tehtud kättesaadavaks [Eesti Avaandmete Portaalis](https://andmed.eesti.ee/). Ka enamikul ELi liikmesriikidel on oma avaliku sektori avaandmete portaalid.  
...
Tartu Ülikool pakub TÜ raamatukogu kaudu võimalust säilitada oma teadusandmeid [DataDOI](https://datadoi.ee/) repositooriumis. Seal saavad andmed endale digitaalobjekti identifikaatori ehk DOI ning need registreeritakse koos metaandmetega rahvusvahelises DataCite registris. [DataCite Eesti](https://datacite.ee/) koondab ka teisi Eesti teadusega seotud repositooriume. Raamatukogu on teinud ka põhjaliku [ülevaate](https://utlib.ut.ee/et/sisu/avatud-teadus) avatud teaduse repositooriumidest, avatud juurdepääsuga teadusandmebaasidest, andmehaldusest, kursustest, üritustest jm-st.
...
[Statistikaamet](https://www.stat.ee/) pakub avalikke andmeid sageli seotult mingite haldusüksuste andmetega (nt maakonnapõhiselt). Statistikaameti teemakaarte saab vaadata [kaardirakenduse](https://xgis.maaamet.ee/xgis2/page/app/statistika) kaudu. Samuti on võimalik andmeid lihtsalt alla laadida ning haldusüksuse tulba kaudu ühendada Maa-ameti ruumiandmetega (nt saab statistikaameti maakondade näitajad siduda Maa-ameti vastavate maakondade polügoonidega).   
...
Samuti saab statistikaameti lehelt alla laadida nt Eesti rahvastiku paiknemise ruutkaardi, mis kujutab rahvaarvu 1 ruutkilomeetri kohta 2024. aasta andmete põhjal.  
```{r statametruutkaart, echo = F, fig.show='hold', fig.align='center', out.width="80%", fig.cap = "Eesti rahvastiku paiknemine statistikaameti ruutkaardil (2024)"}
knitr::include_graphics(here::here("imgs", "5_statruutkaart.png"))
...
- Ruumiandmeid leiab mingil määral [Eesti Avaandmete Portaalist](https://andmed.eesti.ee/datasets?type=ALL), sh Maa-ameti ja Statistikaameti andmeid, aga ka näiteks kohalike omavalitsuste, registrite, ametite jt avaliku sektori institutsioonide andmeid.   
...
- Nii Eesti kui ka välisriikide praeguseid ja endiseid nimekujusid leiab nii Eesti Keele Instituudi [Kohanimeraamatust](https://www.eki.ee/dict/knr/) kui ka [Kohanimeandmebaasist](https://www.eki.ee/knab/knab.htm). Mõlema puhul on võimalik näiteks teadustöö eesmärgil saada enda kasutusse ka lähteandmeid (või osa nendest). Ametlikku [kohanimeregistrit](https://geoportaal.maaamet.ee/est/teenused/aks-kohanimeregistri-teenus-p133.html), mis esitab kohanimede ametliku, standardse nimekuju, haldab Maa-amet. 
- [TARA (Tartu Ülikooli arheoloogia infosüsteem)](http://tara.ut.ee/): leidude, luude, proovide, arhiivi, fotode jms kogud.    
- [Kultuurimälestiste register](https://register.muinas.ee/public.php?menuID=monument): informatsioon kõikide Eestis riikliku kaitse all olevate mälestiste kohta, päringutulemustel sageli juures ka koordinaadid.  
- [Uurali keelte geograafiline andmebaas](https://zenodo.org/records/4784188): Uurali keelte ruumiandmed ja valmis kaardid.  
...
- [Open Maps for Europe](https://www.mapsforeurope.org/): vektor- ja rasterandmed, veebiteenused.   
...
Geoinfoga varustatud andmeid leiab ka mitmesugustelt üldistelt lehekülgedelt ja platvormidelt, mis võimaldavad andmeid koguda oma API (*Application Programming Interface*) abil. Samuti saab ruumiandmeid koguda nt [Wikidata](https://www.wikidata.org/wiki/Wikidata:Main_Page) andmebaasist.  



### Harjutus

- Laadime Natural Earth Data lehelt alla maailma riikide polügoonide vektorandmed, mõõtkavaga 1:50 000 000 (`Downloads → Medium scale data → Cultural → Admin 0 - Countries`). Pakime allalaaditud faili lahti ning loeme SHP-faili QGISi (NB! Pane tähele CRS-i).    
...
- Laadime QGISi Maa-ameti [maakonnapiiride vektorkihi](https://s3.pilw.io/rp-kemit-kataster/EHAK/maakond_shp.zip) ja Moodle'is oleva CSV-andmestiku [statistikaameti rahvaloenduse andmetega](https://andmed.stat.ee/et/stat/rahvaloendus__rel_vordlus__rahvastiku_paiknemine/RLV003) (NB! Statistikaameti andmetes ei ole koordinaate -> `No geometry`).  
...
- `Logarithmic Scale` - rohkem tuuakse esile erinevusi väikeste väärtuste vahel (NB! ei tööta negatiivsete väärtustega)    
...
```{r lohmus, echo = F, fig.align="center", fig.show='hold', out.width = "70%", fig.cap = ""}
knitr::include_graphics(here::here("imgs", c("5_lohmusparn.png")))
``` 
