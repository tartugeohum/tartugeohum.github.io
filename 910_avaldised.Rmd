...
-   CSV-faili, mis sisaldab andmeid *ei ole* ja *pole* suhtelisest **sagedusest** [Eesti murrete korpuses](http://www.murre.ut.ee/mkweb/). NB! Kodeering on UTF-8, välju eraldab semikoolon, esimesel real on tulbanimed ning koordinaatide infot failis ei ole.  
...
-   sümbolite, siltide või paigutuse parameetrite muutmisel (*Symbology → Rule-based*) ![](imgs/10_rulebased.png),  
-   jne.



Avaldisi saab eeldefineeritud funktsioonide abil koostada niisiis mitmeks erinevaks otstarbeks, vaata ülevaadet [QGIS 3.22 juhendist](https://docs.qgis.org/3.22/en/docs/user_manual/expressions/expression.html). Sealjuures saab üht ja sama asja teha sageli erineval moel.  

Oleme juba lihtsate avaldistega kokku puutunud, kui

-   [filtreerisime aadressiandmete andmestikust välja ainult eluruumid](#p8h1): `"ADOB_LIIK" = 'ER'`;
-   tegime tekstina sisse loetud arvude tulbast arvulise tunnuse (\@ref(fig:addnumericfield1)): `to_int("VAARTUS")`;
-   arvutasime digiteeritud polügoonide pindala (\@ref(fig:koilapindala)): `round($area / 10000, 2)`.
...
- Valime kihelkondade kihi ***Symbology*** alt *Graduated* ja *Value* puhul ühe konkreetse tunnuse nime valiku asemel vajutame lahtri kõrval olevat avaldise sümboliga nuppu ![](imgs/10_avaldisenupp.png).  
...
- Alloleva kaardi saamiseks teeme veel mõned kujunduslikud sammud:  
    ...
    - *Symbology → Labels*: 
        ...
        - näitame kõiki (ka kattuvaid) silte (*Rendering → Show all labels for this layer (including colliding labels)*).  
...
- Valime nüüd punktide kihi ***Symbology*** alt *Categorized* ja *Value* puhul vajutame jälle avaldise sümboliga nuppu.  
    ...
    - Selles avaldises ütleme, et kujunda mingit moodi ainult need punktid, mille ridadel atribuuttabelis on tulbas "ANIMI" sõna alguses *Jõe* (millele võib järgneda ükskõik mis, seda märgib sümbol `%`), sõna lõpus *jõe* (millele võib eelneda ükskõik mis), sõna lõpus *jõe*, millele järgneb tühik ja sellele omakorda ükskõik mis (nt *...jõe küla*), samamoodi *järve* kohta.  
    - Märgime sellised objektid näiteks tähekestega ja muud punktid, mis avaldisele ei vasta, helehalliks.  
- Lisame sama avaldist kasutades ka sildid ainult valitud punktidele: **`if("ANIMI" ILIKE 'Jõe%' OR "ANIMI" ILIKE '%jõe' OR "ANIMI" ILIKE '%jõe %' OR "ANIMI" ILIKE 'Järve%' OR "ANIMI" ILIKE '%järve' OR "ANIMI" ILIKE '%järve %', "ANIMI", NULL)`**.  
...
SQL-i päringuid saab QGISis teha nt ***DB Manager***'i tööriistaga. Mõned SQL-i funktsioonid (nt `UPDATE`) töötavad ainult "päris" andmebaasiformaatidega, nt *GeoPackage*'i failidega (kui need on ka *GeoPackage*'ina sisse loetud, mitte lihtsalt vektorfailidena).  
...
- Teeme SQLi päringu teel virtuaalse koopia kihelkondade kihist (nimetame selle nt *kihelkonnad2*). Nõnda ei mõjuta see, mida edasi teeme, meie algandmestikku.  
