...
Avame QGISi. QGISis on georefereerimise tööriist *Georeferencer* leitav ülevalt menüüst *Raster* alt (alates QGISi versioonist 3.26 *Layer* alt). Juhul, kui seal valikut *Georeferencer* ei ole (näiteks kui kasutad vanemat QGISi versiooni), siis tuleb see aktiveerida. Selleks vali *Plugins → Manage and Install Plugins* → otsingusse kirjuta *Georeferencer* → tee linnuke *Georefencer GDAL Plugin* ette ja sulge see aken. Nüüd on tööriist *Georeferencer* rastrimenüü all näha. Tööriista sisselülitamine on ühekordne tegevus, järgmisel korral QGISi avades on see juba olemas. Selle tööriista kohta saab täpsemalt lugeda [QGIS 3.22 juhendist](https://docs.qgis.org/3.22/en/docs/user_manual/working_with_raster/georeferencer.html).
...
Georefereerimiseks on vaja aluskaarti, mille pealt leida ühised punktid ajaloolise kaardiga. Selleks avame Maa-ameti WMS-teenusest *Hybriidkaart*, *Põhikaart* ja *Ortofoto*.
...
Vaatame, kui hästi tänapäevase koordinaatsüsteemiga sidumine õnnestus. Kui kaart on väga paigast ära, siis saame lisada rohkem punkte või muuta transformatsiooni tüüpi. Vastavatest algoritmidest saab täpsemalt lugeda tööriista *Georeferencer* [juhendist](https://docs.qgis.org/3.22/en/docs/user_manual/working_with_raster/georeferencer.html#available-transformation-algorithms), ent lühidalt:  

- *Linear* algoritm sobib põhiliselt siis, kui tegu on kõrge kvaliteediga rasterkaardiga, mille koordinaatide referentssüsteem on teada. See ei võimalda kujutist pöörata ega muul moel muundada. Enamasti sobib see seega uuematele kaartidele, mis on (kindlas projektsioonis) suure täpsusega.     
- *Helmerti* algoritm sobib hea kvaliteediga suuremõõtkavalistele rasterkaartidele või ortofotodele, mille koordinaatsüsteem ei pruugi ühtida aluskaardi omaga. Võimaldab kujutist ka pöörata ja muuta mõõtkava.   
...
[georeferencer.com](http://www.georeferencer.com/) töötab sisuliselt täpselt samamoodi nagu QGISi Georeferencer, aga võimaldab georefereerida internetis ja ilma spetsiaalse tarkvarata. 


## Järgmisel korral

Neljapäevases praktikumis vaatame lähemalt, kuidas saab QGISis ruumiandmetega töötamisel kasutada avaldisi ja päringuid.
