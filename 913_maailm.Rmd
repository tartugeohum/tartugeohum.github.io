...
- valime *Vector → Geometry Tools → Multipart to singleparts*,  
...
- valime *Vector → Geometry Tools → Collect Geometries*,  
...
Värvime esmalt polügoonid avaldise abil, kus jagame GDP rahvaarvuga ning jaotame need väärtused sagedusklassidesse. Vaatame esmalt atribuuttabelit. Kas tahame näidata kõiki andmeid?
Vatikani GPD real on arv -99. See ei tähenda, et Vatikani SKP oleks negatiivne, vaid nii märgitakse tabelis puuduvaid väärtusi. Võiksime niisiis Vatikani kaardil kuidagi eraldi kujutada. Samuti võiksime analüüsimata jätta Venemaa andmed.  


### Ülesanne: polügoonide värvimine SKP/in põhjal

Polügoonide värvimiseks selle põhjal, kui suur on vastava riigi SKP inimese kohta, võib lihtsalt kasutada avaldist, aga võime ka esmalt teha andmestikku uue ajutise tulba, et hiljem oleks vähem trükkimist. Uue tulba nimi võiks olla `gdp_pc` (*GDP per capita*), tüüp kümnendarv (*decimal number*) ja sinna võiksid minna väärtused, mille leiame avaldisega `"GDP_MD" / "POP_EST" * 1000000`. Vatikani ja Venemaa osade väärtused võiksid olla `NULL`. 
...
Praegusel juhul oleks tegelikult võinud kujundada diagrammide asemel ja lihtsalt mingit polügoonide keskpunktide (*Centroids*) kihti.  

Kui aga tahta lihtsalt graafikuid, saab sedagi QGISis edukalt teha. Selleks võib kasutada *DataPlotly* pistikprogrammi (*Plugins → Manage and install plugins*). Proovime teha tulpdiagrammi (*Bar Plot*) riikide rahvaarvust ja sektordiagrammi Euroopa 4 eri regiooni rahvaarvust. 
